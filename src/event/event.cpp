...
void Event::run() {
  while(user->getCurrentHealth() > 0 && sumEnemiesHealth() > 0) {
   wclear(eventBox);
...
int Event::sumEnemiesHealth() {
   return enemy1->getCurrentHealth() + enemy2->getCurrentHealth() + enemy3->getCurrentHealth();
}
